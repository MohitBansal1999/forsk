...
for row in right_table.find_all('tr'):
    ...
    if len(cells) == 7 and count<6:
        ...
        share.append(float(cells[4].text.strip()))
        count=count+1
...
colors = ['gold', 'yellowgreen', 'lightcoral','green','red','cyan']
explode = [0, 0, 0,0,0]
i=share.index(np.max(share))
explode.insert(i,0.1)
plt.pie(share, explode=explode, labels=state, colors=colors, autopct='%1.1f%%', shadow=True, startangle=0)
...
import pandas as pd
import numpy as np
d1=pd.read_csv("Automobile.csv")
car=[]
number=[]
a=d1["make"].value_counts().head(10)
for i in a:
    car.append(a[a == i].index[0])
    number.append(i)
colors = ['gold', 'yellowgreen', 'lightcoral','green','red','cyan','skyblue','grey','blue','pink']  
explode=[0,0,0,0,0,0,0,0,0]
i=number.index(np.max(number))
explode.insert(i,0.1)
plt.pie(number, explode=explode, labels=car, colors=colors, autopct='%1.1f%%', shadow=True, startangle=0)
...
"""
df=pd.read.("baby_names\yob1880.txt")
df["year"]=1880
for i in range(1881,2011):
    df1=pd.read_csv("baby_names\yob"+str(i)+".txt")
    df1["year"]=i
    df=pd.concat([df, df1])
i=1881
df1=pd.read_csv("baby_names\yob"+str(i)+".txt")
