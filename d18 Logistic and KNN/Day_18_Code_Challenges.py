...
affair = pd.read_csv('affairs.csv', sep=',', header=0)  
labels = affair.iloc[:,8].values 
features = affair.iloc[:,:8].values
...
x=np.array([3,25,3,1,1,16,4,2]).reshape(1,8)
x = onehotencoder.transform(x).toarray()
x = x[:, 1:]
x = onehotencoder1.transform(x).toarray()
x = x[:, 1:]
classifier.predict(sc.transform(x))
...
mushrooms = pd.read_csv('mushrooms.csv', sep=',', header=0)  
labels = mushrooms.iloc[:,0].values 
features = mushrooms.iloc[:,1:].values
-----
##using all columns in features
...
from sklearn.preprocessing import OneHotEncoder
for i in range(22):
    onehotencoder = OneHotEncoder(categorical_features = [features.shape[1] - 1])
    features = onehotencoder.fit_transform(features).toarray()
    features = features[:, 1:]
...
-------
#using habitat,population and odor in feature
'''features=mushrooms.iloc[:,21:].values
